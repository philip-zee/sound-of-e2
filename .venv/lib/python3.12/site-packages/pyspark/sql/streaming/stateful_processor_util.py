...
class TransformWithStateInPandasUdfUtils:
    ...
    def _transformWithStateUDF_realtime_mode(
        ...
        if mode == TransformWithStateInPandasFuncMode.PROCESS_TIMER:
            ...
            # BEGIN-EDGE
            if stateful_processor_api_client.worker_cache_enabled:
                stateful_processor_api_client.flush_request_batch_and_remove_implicit_key()
            else:
                # END-EDGE
                stateful_processor_api_client.remove_implicit_key()
            self._stateful_processor.close()
    ...
    def _handle_data_rows(
        ...
        # BEGIN-EDGE
        if (
            stateful_processor_api_client.is_realtime_mode and
            stateful_processor_api_client.worker_cache_enabled
        ):
            # Process any pending batch requests.
            stateful_processor_api_client.flush_request_batch_and_set_implicit_key(key)
        else:
            # END-EDGE
            stateful_processor_api_client.set_implicit_key(key)
    ...
    def _handle_expired_timers(
        ...
        for key_obj, expiry_timestamp in expiry_iter:
            ...
            # BEGIN-EDGE
            if (
                stateful_processor_api_client.is_realtime_mode
                and stateful_processor_api_client.worker_cache_enabled
            ):
                stateful_processor_api_client.flush_request_batch_and_set_implicit_key(key_obj)
            else:
                # END-EDGE
        ...
        if (
            ...
            # flush any pending requests after processing timers
            stateful_processor_api_client.build_and_flush_request_batch()
        # END-EDGE
