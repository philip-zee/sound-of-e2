...
class StatefulProcessorApiClient:
    def __init__(
        ...
        self.worker_cache_enabled = worker_cache_enabled # EDGE
        # BEGIN-EDGE
        ...
        self.value_state_write_buffer: Dict[str, ValueStateWriteBufferEntry] = {}
        # Dict to store map state write buffer with type
        # {state_name, MapStateWriteBufferEntry}
        self.map_state_write_buffer: Dict[str, MapStateWriteBufferEntry] = {}
        # List to store timer state write buffer entries
        self.timer_write_buffer: List[TimerWriteBufferEntry] = []
    ...
    def register_timer(self, expiry_time_stamp_ms: int) -> None:
        import pyspark.sql.streaming.proto.StateMessage_pb2 as stateMessage
        # BEGIN-EDGE
        if self.worker_cache_enabled and self.is_realtime_mode:
            self.timer_write_buffer.append(
                TimerWriteBufferEntry(
                    expiryTimestampMs=expiry_time_stamp_ms,
                    operation=TimerWriteBufferOperation.REGISTER
                )
            )
            return
        # END-EDGE
    ...
    def delete_timer(self, expiry_time_stamp_ms: int) -> None:
        ...
        if self.worker_cache_enabled and self.is_realtime_mode:
            self.timer_write_buffer.append(
                TimerWriteBufferEntry(
                    expiryTimestampMs=expiry_time_stamp_ms,
                    operation=TimerWriteBufferOperation.DELETE
                )
            )
            return
        # END-EDGE
    ...
    def _build_request_batch_from_write_buffer(self) -> None:
        import pyspark.sql.streaming.proto.StateMessage_pb2 as stateMessage
        if self.value_state_write_buffer:
            for state_name, value_state_entry in self.value_state_write_buffer.items():
                if value_state_entry is None:
                    ...
                    bytes = self._serialize_to_bytes(
                ...
                self.request_batch.append(message)
            self.value_state_write_buffer.clear()
        if self.map_state_write_buffer:
            for state_name, map_state_entry in self.map_state_write_buffer.items():
                if map_state_entry is None:
                    ...
                    for user_key, value in user_key_value_pairs.items():
                        if value is None:
                            key_bytes = self._serialize_to_bytes(user_key_schema, user_key)
                            ...
                            key_bytes = self._serialize_to_bytes(user_key_schema, user_key)
                            value_bytes = self._serialize_to_bytes(value_schema, value)
                        ...
                        self.request_batch.append(message)
            self.map_state_write_buffer.clear()
        if self.timer_write_buffer:
            for timer_entry in self.timer_write_buffer:
                ...
                if operation == TimerWriteBufferOperation.REGISTER:
                    ...
                    message = stateMessage.StateUnitRequest(statefulProcessorCall=call)
                    self.request_batch.append(message)
            self.timer_write_buffer.clear()

    def flush_request_batch_and_set_implicit_key(self, key: Optional[Tuple]) -> None:
        import pyspark.sql.streaming.proto.StateMessage_pb2 as stateMessage
        ...
        self.request_batch.append(message)
        self.flush_request_batch()

    def flush_request_batch_and_remove_implicit_key(self) -> None:
        import pyspark.sql.streaming.proto.StateMessage_pb2 as stateMessage
        self._build_request_batch_from_write_buffer()
        remove_implicit_key = stateMessage.RemoveImplicitKey()
        request = stateMessage.ImplicitGroupingKeyRequest(removeImplicitKey=remove_implicit_key)
    ...
    def flush_request_batch(self) -> None:
        import pyspark.sql.streaming.proto.StateMessage_pb2 as stateMessage
        if not self.request_batch:
            return
        batch_call = stateMessage.BatchRequest(
            stateUnitRequests=self.request_batch
        )
    ...
    def build_and_flush_request_batch(self) -> None:
        self._build_request_batch_from_write_buffer()
        self.flush_request_batch()
    # END_EDGE
...
class ListTimerIterator:
    def __init__(self, stateful_processor_api_client: StatefulProcessorApiClient):
        ...
        self.stateful_processor_api_client.build_and_flush_request_batch() # EDGE
...
# BEGIN-EDGE
class TimerWriteBufferOperation(Enum):
    """
    Internal timer operation to be stored in the write buffer.
    """
    REGISTER = 1
    DELETE = 2
...
@dataclass
class ValueStateWriteBufferEntry:
    """
    Value state entry stored in the write buffer. It contains the pending values to be written
    back to the server.
    :param value: the value of the state.
    :type value: Tuple
    :param schema: the schema of the value state.
...
@dataclass
class MapStateWriteBufferEntry:
    """
    Map state entry stored in the write buffer. It contains the pending user_key,value pairs
    to be written back to the server.
    :param user_key_value_pairs: the user_key,value pairs of the state. If the value is None,
    it means remove_key() was called on the corresponding user_key.
    :type user_key_value_pairs: Dict[Tuple, Optional[Tuple]]
    :param user_key_schema: the schema of the user_key in map state.
    :type schema: StructType
    :param value_schema: the schema of the value in map state.
    :type schema: StructType
    """
    user_key_value_pairs: Dict[Tuple, Optional[Tuple]]
    user_key_schema: StructType
    value_schema: StructType


@dataclass
class TimerWriteBufferEntry:
    """
    Timer entry stored in the write buffer. It contains the pending expiryTimestampMs to be
    registered or deleted on the server.
    :param expiryTimestampMs: the expiry timestamp in ms.
    :type expiryTimestampMs: int
    :param operation: the operation to be performed on the timer, either REGISTER or DELETE.
    :type operation: TimerWriteBufferOperation
    """
    expiryTimestampMs: int
    operation: TimerWriteBufferOperation
# END-EDGE
